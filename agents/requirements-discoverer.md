...
```bash
uv run --script <plugin-root>/skills/jira-reader/scripts/jira_reader.py --issue <TICKET>
```
...
```bash
uv run --script <plugin-root>/skills/jira-reader/scripts/jira_reader.py --graph <TICKET> --max-graph-tokens 15000
```
...
```bash
uv run --script <plugin-root>/skills/jira-reader/scripts/jira_reader.py \
...
uv run --script <plugin-root>/skills/git-pr-reader/scripts/git_pr_reader.py diff <pr-url> --save-diff <OUTPUT_DIR>/pr-<number>.diff
```
...
```bash
uv run --script <plugin-root>/skills/docs-convert-gdoc-md/scripts/gdoc2md.py --manifest --split-sections "<google-doc-url>" <OUTPUT_DIR>/spec-<id>.md
```
