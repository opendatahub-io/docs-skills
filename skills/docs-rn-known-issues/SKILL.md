...
If the Atlassian MCP is unavailable or unauthenticated, fall back to docs-jira-reader:

```bash
# Claude Code
uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-jira-reader/scripts/jira_reader.py --jql "<JQL>" --fetch-details

# Cursor
uv run --script skills/docs-jira-reader/scripts/jira_reader.py --jql "<JQL>" --fetch-details
```
...
> "Run docs-rn-known-issues for rhoai-3.5 --project RHOAIENG"
