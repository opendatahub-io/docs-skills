---
name: docs-understand-pull-request
description: Analyze a pull request or merge request. Fetches PR metadata via docs-git-pr-reader, identifies affected modules, analyzes changes in context, and produces a PR-ANALYSIS.md document with a brief repo overview and detailed change analysis.
...
```
/docs-skills:docs-understand-pull-request 42
/docs-skills:docs-understand-pull-request 42 --repo /path/to/repo
/docs-skills:docs-understand-pull-request https://github.com/org/repo/pull/42
/docs-skills:docs-understand-pull-request https://gitlab.com/org/repo/-/merge_requests/42
```
...
The docs-git-pr-reader script requires API tokens. Verify that the relevant token is set:
...
Write to `${BASE_PATH}/workflow/understand-pr_${REPO_NAME}_${PR_NUMBER}.json`. Fields: `workflow` ("docs-understand-pull-request"), `target`, `pr_number`, `platform`, `repo_path`, `base_path`, `pr_base` (all absolute paths), `status` ("in_progress"), `created_at`/`updated_at` (ISO 8601 UTC), `step_order` (["pr-metadata", "repo-context", "change-analysis", "synthesis"]), `steps` (each with `status: "pending"`, `output: null`, `result: null`).
...
```bash
GIT_PR_READER="${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py"
...
Use the docs-learn-code detection scripts (shared across skills):

```bash
LEARN_CODE_SCRIPTS="${CLAUDE_PLUGIN_ROOT}/skills/docs-learn-code/scripts"
...
Combine detection and module map results into a single detection JSON (same format as docs-learn-code's detection.json). Write to `${OUTPUT_DIR}/detection.json`.

### 2.6 Produce repo overview

Check if a prior docs-learn-code analysis exists:
...
Log: `"Reusing repo overview from existing docs-learn-code analysis"`.
...
No detailed overview available. Run /docs-skills:docs-learn-code for a full analysis.
```

### 2.7 Write step-result.json

Write to `${OUTPUT_DIR}/step-result.json` with: `schema_version: 1`, `step: "repo-context"`, `target`, `completed_at`, `primary_language`, `module_count`, `overview_source` (one of "docs-learn-code", "pr-repo-summarizer", "fallback").
