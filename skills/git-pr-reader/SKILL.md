...
```bash
uv run --script scripts/git_pr_reader.py read --url "https://github.com/owner/repo/pull/123"
uv run --script scripts/git_pr_reader.py read --url "https://gitlab.com/group/project/-/merge_requests/456" --format markdown
uv run --script scripts/git_pr_reader.py read --url "https://github.com/owner/repo/pull/123" --no-filter
```

#### info — Get PR/MR information

```bash
uv run --script scripts/git_pr_reader.py info https://github.com/owner/repo/pull/123 --json
```

#### files — List changed files

```bash
uv run --script scripts/git_pr_reader.py files https://github.com/owner/repo/pull/123
uv run --script scripts/git_pr_reader.py files https://github.com/owner/repo/pull/123 --filter "*.adoc" --json
```

#### comments — List review comments

```bash
uv run --script scripts/git_pr_reader.py comments https://github.com/owner/repo/pull/123
uv run --script scripts/git_pr_reader.py comments https://github.com/owner/repo/pull/123 --include-resolved --json
```

#### diff — Get unified diff

```bash
uv run --script scripts/git_pr_reader.py diff https://github.com/owner/repo/pull/123
```

#### post — Post review comments

```bash
uv run --script scripts/git_pr_reader.py post https://github.com/owner/repo/pull/123 comments.json
uv run --script scripts/git_pr_reader.py post https://github.com/owner/repo/pull/123 comments.json --review-type technical
uv run --script scripts/git_pr_reader.py post https://github.com/owner/repo/pull/123 comments.json --review-type style --dry-run
```
...
uv run --script scripts/git_pr_reader.py reply https://github.com/owner/repo/pull/123 --comment-id 12345 --body "Applied the fix."

# Reply to a GitLab discussion
uv run --script scripts/git_pr_reader.py reply https://gitlab.com/group/project/-/merge_requests/456 --discussion-id "abc123" --body "Fixed." --signoff "CI bot"

# Dry run
uv run --script scripts/git_pr_reader.py reply https://github.com/owner/repo/pull/123 --comment-id 12345 --body "Test" --dry-run
```
...
uv run --script scripts/git_pr_reader.py extract https://github.com/owner/repo/pull/123 path/to/file.adoc "pattern"

# Dump all added/modified lines
uv run --script scripts/git_pr_reader.py extract --dump https://github.com/owner/repo/pull/123 path/to/file.adoc

# Validate a comments JSON file against the diff
uv run --script scripts/git_pr_reader.py extract --validate https://github.com/owner/repo/pull/123 comments.json
```

#### metadata — Get combined PR/MR metadata

```bash
uv run --script scripts/git_pr_reader.py metadata https://github.com/owner/repo/pull/123
uv run --script scripts/git_pr_reader.py metadata https://gitlab.com/group/project/-/merge_requests/456
uv run --script scripts/git_pr_reader.py metadata https://github.com/owner/repo/pull/123 --diff-output /path/to/diff.patch
```
...
```bash
uv run --script scripts/git_pr_reader.py detect
uv run --script scripts/git_pr_reader.py detect --json
```
