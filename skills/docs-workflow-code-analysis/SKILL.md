...
description: "Run code-learner analysis on a source repository for the docs orchestrator workflow. Dispatches a subagent to run docs-learn-code, keeping the heavy orchestration out of the main context. Produces ONBOARDING.md, module registry, per-module summaries, and cross-module relationship data."
...
Orchestrator step skill that wraps `docs-learn-code` to analyze a source repository and produce structured code understanding for downstream documentation steps.
...
Check if docs-learn-code output already exists. Learn-code may store results in two locations depending on the subagent's working directory:
...
```bash
# Find the docs-learn-code base directory containing the cached analysis
...
### 3. Dispatch docs-learn-code subagent

**You MUST use the Agent tool** to run docs-learn-code in an isolated subagent. Do NOT invoke `Skill: docs-learn-code` inline — that would load 850+ lines of skill text plus all intermediate orchestration into the main context.

```
Agent:
  description: "Run docs-learn-code analysis on <REPO>"
  prompt: |
    Run the docs-learn-code skill to analyze the source repository.

    Skill: docs-learn-code, args: "<REPO>"

    After docs-learn-code completes, report the location of the output files
    (ONBOARDING.md, registry.json, detection.json, summaries/, relationships/).
```

After the agent completes, locate the docs-learn-code output. Check both possible locations (same as step 2):
