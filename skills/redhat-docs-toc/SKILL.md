...
```bash
uv run --script <skill-dir>/scripts/toc_extractor.py --url "https://docs.redhat.com/en/documentation/product/version/html/guide/index"
```

**Save to file:**
```bash
uv run --script <skill-dir>/scripts/toc_extractor.py --url "https://docs.redhat.com/..." --output articles.json
```

**List format output:**
```bash
uv run --script <skill-dir>/scripts/toc_extractor.py --url "https://docs.redhat.com/..." --format list
```
...
uv run --script <skill-dir>/scripts/toc_extractor.py \
...
while read url; do
  ...
  uv run --script <plugin-root>/skills/article-extractor/scripts/article_extractor.py \
    --url "$url" \
    --format markdown \
    --output "$filename"
