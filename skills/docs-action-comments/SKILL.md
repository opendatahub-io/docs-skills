...
| `--base-path <path>` | Base output path (required). Used to **read** workspace artifacts from prior workflow steps (code-analysis, requirements, etc.) and to **write** `step-result.json` sidecar to `${BASE_PATH}/docs-action-comments/` |
...
```bash
PR_URL=$(uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py detect 2>/dev/null)
```
...
```bash
HEAD_REF=$(uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py info "${PR_URL}" --field head_ref)
TITLE=$(uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py info "${PR_URL}" --field title)
```
...
```bash
uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py comments "${PR_URL}" --json
```
...
git -c user.name="docs-action-comments[bot]" \
    -c user.email="docs-action-comments@users.noreply.github.com" \
...
**Routing flag** (from the Step 4 JSON): GitHub → `--comment-id "${COMMENT_ID}"` (the comment's `id`); GitLab → `--discussion-id "${DISCUSSION_ID}"`. Keep `--signoff` exactly `Claude Code docs-action-comments (CI)` — that string is how `has_bot_reply` detects prior replies for idempotency.

```bash
uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/docs-git-pr-reader/scripts/git_pr_reader.py \
  reply "${PR_URL}" ${ROUTING_FLAG} \
  --body "${REPLY_BODY}" --signoff "Claude Code docs-action-comments (CI)"
```
...
Comments applied without workspace evidence still count as resolved (`--comments-resolved`); the note lives in the reply, not a separate counter.

Pass `--ci-mode` in `${CI_MODE_FLAG}` when `ci_mode` is true, which sets `comments_replied` to the number of replies posted. Omit it interactively, where that count is `0`.
