...
```bash
python3 <skill-dir>/scripts/detect_language.py --repo <REPO_PATH>
```
...
```bash
python3 <skill-dir>/scripts/build_module_map.py --repo <REPO_PATH> --lang <PRIMARY_LANGUAGE> [--exclude <PATTERNS>...]
```
...
```bash
python3 <skill-dir>/scripts/classify_modules.py \
...
For each module, run the language-appropriate AST extraction: Python uses `extract_public_api.py --files <files> --lang python --module <name>`. Go/JS/TS use `uv run --script <skill-dir>/scripts/extract_public_api_treesitter.py --files <files> --lang <lang> --module <name>`. Log warning and continue if extraction fails for a module.
...
```bash
python3 <skill-dir>/scripts/build_dep_pairs.py \
...
Read language-specific relationship analysis guidance from `<plugin-root>/reference/language-configs.md`.
...
```bash
python3 <skill-dir>/scripts/build_synthesis_context.py \
  --base-path "${BASE_PATH}" \
...
Dispatch `subagent_type: docs-skills:synthesis-writer`. The context is written to `${OUTPUT_DIR}/context.json` — the agent reads it from disk. Tell the agent to write ONBOARDING.md (and dependency-graph.json if relationships exist) to OUTPUT_DIR, following the template from `<plugin-root>/reference/onboarding-template.md`.
