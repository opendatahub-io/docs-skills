...
- `--no-source-repo` — Skip source repo resolution and all source-dependent steps (code-analysis, and scope-req-audit when `--with-scope-audit` is used). The workflow runs without source grounding. Use for tickets with no associated source code repository, or pass on resume after the workflow stops due to no repo being found
- `--with-scope-audit` — Include the scope-req-audit step in the workflow, even though it is not in the default step list. Adds scope-req-audit after code-analysis with `when: has_source_repo`. Use for EA/preview features where pre-writing code-evidence classification justifies the cost (~13 min, 8 agents). Without this flag, quality-gate performs a lightweight inline evidence check using the module registry + grep
- `--auto-discover-repos` — Skip the confirmation prompt when secondary repos are discovered by scope-req-audit. Only relevant when `--with-scope-audit` is used. Useful for CI/automation where interactive prompts are not available. Has no effect if no secondary repos are found
- `--max-secondary-repos <N>` — Maximum number of secondary repos to clone after scope-req-audit (default: 3). Only relevant when `--with-scope-audit` is used. Repos are ranked by the number of associated requirements
