...
Same argument set as docs-orchestrator. See [argument reference](../docs-orchestrator/references/argument-reference.md) for full descriptions.

- `$1` — JIRA ticket ID (optional at this stage)
- `--workflow <name>` — Named workflow YAML
- `--pr <url>...` — PR/MR URLs
- `--mkdocs` — MkDocs format instead of AsciiDoc
- `--draft` — Write to staging area only
- `--docs-repo-path <path>` — Target docs repo for update-in-place
- `--source-code-repo <url-or-path>...` — Source code repos
- `--create-merge-request` — Open MR/PR after reviews
- `--no-source-repo` — Skip source repo resolution
- `--auto-discover-repos` — Skip secondary repo confirmation
- `--max-secondary-repos <N>` — Max secondary repos (default: 3)
