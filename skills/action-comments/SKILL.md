...
Fetch unresolved review comments from a GitHub PR or GitLab MR and action them on local files. Runs interactively by default, or autonomously in CI (auto-detected, or forced with `--ci`).
...
In CI mode, **ignore `--include-resolved`** — re-actioning already-resolved threads is never wanted autonomously.
...
Set `WORKSPACE` to `workspace`. If `source_repo` is non-null, set `SOURCE_REPO` to it — this enables direct code lookups when verifying reviewer comments.

**Load artifacts** (for each artifact reported `true`) — read the file at the path below:
...
| Source config | `${WORKSPACE}/source.yaml` | Source repo path (already resolved into `SOURCE_REPO` above) — use for direct code verification |
...
Add `--include-resolved` only in interactive mode if the `--include-resolved` flag was passed (ignore it in CI mode — see Step 0).

The script automatically filters bot comments, resolved threads (unless `--include-resolved`), and returns top-level comments with: `id`, `path`, `line`, `body`, `author`, `resolved`, `has_bot_reply`, `position_outdated`. For GitLab MRs each comment also carries `discussion_id`.

**Idempotency (CI cron):** in CI mode, **skip any comment where `has_bot_reply` is `true`** — it already received an action-comments reply on a prior run. This is what makes repeated cron runs safe. Save the raw JSON to a file (e.g. `comments.json`) for the next step.
...
**Outdated detection** — annotate each comment with an `outdated` flag using the forge's own staleness signal (`position_outdated`) plus a file-existence check:
...
The script echoes the comments JSON with `outdated: true|false` added to each. Auto-skip any comment where `outdated` is `true`.

## Step 6: Process comments

For each non-outdated comment, read the target file and prepare a suggested change grounded in workspace context (see below). Then follow either the **interactive** or **CI** path depending on `ci_mode` (from Step 0).
...
- **Code analysis** (`ONBOARDING.md`): verify reviewer claims about APIs/flags/structure before agreeing or pushing back.
- **Source repo** (`SOURCE_REPO` set): `grep`/`Read` the actual source to verify APIs, config keys, defaults, command syntax — do not guess.
- **Requirements**: if the reviewer suggests adding content, check it's in the original ticket scope; note when it isn't.
- **Technical review**: if the tech review already validated a claim the reviewer questions, cite that validation.
- **Scope audit**: if the requirement is classified `absent` in code, the request may need a "not supported" note instead.
...
**Preconditions** — a CI cron needs a write-scoped `GITHUB_TOKEN` (or `GITLAB_TOKEN`) to push and post replies. If the token is missing, stop early with a clear error rather than applying edits that can't be pushed.
...
After **all** comments are processed and if any files were modified, commit the changes and push them to the PR branch — otherwise the applied edits are lost when the CI job ends. Run once, after the loop:
...
**Routing flag** (from the Step 4 JSON): GitHub → `--comment-id "${COMMENT_ID}"` (use the comment's `id`); GitLab → `--discussion-id "${DISCUSSION_ID}"`. The `--signoff` value must stay exactly `Claude Code action-comments (CI)` — that string is how `has_bot_reply` detects prior replies for idempotency.
...
After all comments are processed, present a summary with: PR/MR URL, branch, workspace grounding (path or "none"); a count table (total, applied, edited, skipped, outdated); a **Changes applied** list (`{path}:{line}` — description); and a **Comments skipped** list (`{path}:{line}` — @author: reason).
