---
name: docs-validate-plugin-prereqs
...
- **`JIRA_API_TOKEN`** / **`JIRA_EMAIL`**: Add to `~/.env` or `.env`. Required for JIRA-related skills (docs-jira-reader, docs-jira-writer, create-jira, jira-ready).
- **`GITHUB_TOKEN`**: Add to `~/.env` or `.env`. Needed for GitHub-based skills (docs-git-pr-reader, docs-understand-pull-request).
...
- **`glab`**: Install from https://gitlab.com/gitlab-org/cli. Required for GitLab workflows (create-merge-request, docs-git-pr-reader).
