...
```bash
python3 scripts/quality_gate.py prepare \
  --ticket "${TICKET}" \
  --base-path "${BASE_PATH}"
```
...
```bash
python3 scripts/quality_gate.py extract-json \
...
- **Output schema**: `schema/doc-quality.json`
...
- **Output schema**: `schema/intent-alignment.json`
...
```bash
python3 scripts/quality_gate.py classify \
  --ticket "${TICKET}" \
  --base-path "${BASE_PATH}" \
...
```bash
python3 scripts/quality_gate.py brief \
  --ticket "${TICKET}" \
  --base-path "${BASE_PATH}" \
  --iteration <N>
```
