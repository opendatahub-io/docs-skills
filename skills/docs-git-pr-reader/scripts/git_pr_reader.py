...
# Signoff appended to replies posted by the docs-action-comments skill in CI mode.
...
# passed by skills/docs-action-comments.
ACTION_COMMENTS_CI_SIGNOFF = "Claude Code docs-action-comments (CI)"
...
class GitHubReviewAPI(GitReviewAPI):
    ...
    def get_review_comments(self, include_resolved: bool = False) -> List[Dict]:
        ...
        # Root comment ids that already have an docs-action-comments CI reply.
        replied_roots: set = {
            c.in_reply_to_id
...
class GitLabReviewAPI(GitReviewAPI):
    ...
    def get_pr_info(self) -> Dict:
        ...
        try:
            ...
            headers = {"User-Agent": "docs-git-pr-reader"}
            if self.token:
