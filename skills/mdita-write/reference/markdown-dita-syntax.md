...
Double-curly keyref syntax is not supported:

```markdown
# NOT SUPPORTED
See {{product_name}} for details.
```

Use standard link syntax or raw DITA `<xref keyref="...">` instead.

### YAML keys block

YAML `keys:` block for map-level key definitions is not supported:
...
Use reference-style links in the map body to create `<keydef>` elements instead.

### Domain class mappings

Inline element class syntax is not supported:

```markdown
# NOT SUPPORTED
Click the [Save]{.uicontrol} button.
```

Use raw DITA XML elements instead: `<uicontrol>Save</uicontrol>`.

### Profiling attributes

Attribute syntax for profiling/conditional processing is not supported:

```markdown
# NOT SUPPORTED
This content is for administrators.{audience="admin"}
```

Use DITA XML `<ph>` or other elements with profiling attributes instead.

### Troubleshooting task section

The "Troubleshooting" H2 section is not recognized as a specialized task element:

```markdown
# NOT SUPPORTED as <tasktroubleshooting>
## Troubleshooting
```

Use regular sections or reference topics for troubleshooting content.
